...
function build() {
  ...
  btnSys.innerHTML = `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><rect x="2" y="3" width="20" height="14" rx="2"/><line x1="8" y1="21" x2="16" y2="21"/><line x1="12" y1="17" x2="12" y2="21"/></svg><span>Sistema</span>`;
  btnSys.title = 'System Optimizer';

  const btnCfgTab = document.createElement('button');
  btnCfgTab.className = 'sidebar-btn';
  btnCfgTab.innerHTML = `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/><polyline points="10 9 9 9 8 9"/></svg><span>Configuração</span>`;
  btnCfgTab.title = 'CFG Manager';

  const btnHwTab = document.createElement('button');
  btnHwTab.className = 'sidebar-btn';
  btnHwTab.innerHTML = `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><rect x="4" y="4" width="16" height="16" rx="2"/><rect x="9" y="9" width="6" height="6"/><line x1="9" y1="1" x2="9" y2="4"/><line x1="15" y1="1" x2="15" y2="4"/><line x1="9" y1="20" x2="9" y2="23"/><line x1="15" y1="20" x2="15" y2="23"/><line x1="20" y1="9" x2="23" y2="9"/><line x1="20" y1="14" x2="23" y2="14"/><line x1="1" y1="9" x2="4" y2="9"/><line x1="1" y1="14" x2="4" y2="14"/></svg><span>Hardware</span>`;
  btnHwTab.title = 'Hardware Info';

  const btnProcTab = document.createElement('button');
  btnProcTab.className = 'sidebar-btn';
  btnProcTab.innerHTML = `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg><span>Processos</span>`;
  btnProcTab.title = 'Process Manager';

  const btnNetTab = document.createElement('button');
  btnNetTab.className = 'sidebar-btn';
  btnNetTab.innerHTML = `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><circle cx="12" cy="12" r="10"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/></svg><span>Rede</span>`;
  btnNetTab.title = 'Network Diagnostics';

  const btnDemoTab = document.createElement('button');
  btnDemoTab.className = 'sidebar-btn';
  btnDemoTab.innerHTML = `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><polygon points="5 3 19 12 5 21 5 3"/></svg><span>Demonstrações</span>`;
  btnDemoTab.title = 'Demo Review';

  const btnAgentTab = document.createElement('button');
  btnAgentTab.className = 'sidebar-btn';
  btnAgentTab.innerHTML = `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg><span>Assistente IA</span>`;
  btnAgentTab.title = 'AI Voice Assistant';
  ...
  btnRankTab.innerHTML = `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg><span>Rankings</span>`;
  ...
  btnServersTab.innerHTML = `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><rect x="2" y="2" width="20" height="8" rx="2" ry="2"/><rect x="2" y="14" width="20" height="8" rx="2" ry="2"/><line x1="6" y1="6" x2="6.01" y2="6"/><line x1="6" y1="18" x2="6.01" y2="18"/></svg><span>Servidores</span>`;
  ...
  btnMarketTab.innerHTML = `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg><span>Mercado</span>`;
  ...
  sidebar.appendChild(btnAgentTab);
  ...
  type TabId = 'sys' | 'cfg' | 'hw' | 'proc' | 'net' | 'demo' | 'agent' | 'rank' | 'servers' | 'market' | 'hub';
  const allBtns = [btnSys, btnCfgTab, btnHwTab, btnProcTab, btnNetTab, btnDemoTab, btnAgentTab, btnRankTab, btnServersTab, btnMarketTab, btnHubTab];
  const tabIds: { btn: HTMLButtonElement; tabId: string }[] = [
    ...
    { btn: btnAgentTab, tabId: 'tab-agent' },
  ...
  function switchTab(t: TabId) {
    const idx = ['sys','cfg','hw','proc','net','demo','agent','rank','servers','market','hub'].indexOf(t);
  ...
  btnAgentTab.addEventListener('click', () => switchTab('agent'));
  ...
  headerRight.appendChild(btnMax);
  ...
  /* ── Tab: AI Agent ──────────────────────────────────────────────── */
  const tabAgent = document.createElement('div');
  tabAgent.id = 'tab-agent';
  tabAgent.className = 'tab-panel';

  // Agent header with provider selector
  const agentHeader = document.createElement('section');
  agentHeader.style.cssText = 'display:flex;gap:8px;padding:10px 12px;align-items:center;flex-wrap:wrap;';

  const providerLabel = document.createElement('span');
  providerLabel.style.cssText = 'font-size:10px;opacity:0.5;font-weight:600;';
  providerLabel.textContent = 'LLM Provider:';

  const providerSelect = document.createElement('select');
  providerSelect.id = 'agent-provider';
  providerSelect.style.cssText = 'padding:4px 8px;border-radius:4px;background:rgba(0,0,0,0.3);border:1px solid rgba(255,255,255,0.1);color:var(--text);font-size:11px;';
  providerSelect.innerHTML = '<option value="">Loading...</option>';

  const voiceToggle = document.createElement('button');
  voiceToggle.id = 'agent-voice-toggle';
  voiceToggle.className = 'btn-import';
  voiceToggle.innerHTML = '🎤 Voice: OFF';
  voiceToggle.title = 'Toggle voice assistant (Ctrl+Space to talk)';

  const statusDot = document.createElement('span');
  statusDot.id = 'agent-status';
  statusDot.style.cssText = 'width:8px;height:8px;border-radius:50%;background:#64748b;margin-left:auto;';
  statusDot.title = 'Agent status';

  const btnClearChat = document.createElement('button');
  btnClearChat.className = 'btn-export';
  btnClearChat.textContent = 'Clear Chat';
  btnClearChat.title = 'Clear conversation history';

  agentHeader.appendChild(providerLabel);
  agentHeader.appendChild(providerSelect);
  agentHeader.appendChild(voiceToggle);
  agentHeader.appendChild(statusDot);
  agentHeader.appendChild(btnClearChat);
  tabAgent.appendChild(agentHeader);

  // Chat messages container
  const chatContainer = document.createElement('div');
  chatContainer.id = 'agent-chat';
  chatContainer.style.cssText = 'flex:1;overflow-y:auto;padding:12px;display:flex;flex-direction:column;gap:12px;min-height:400px;max-height:600px;';
  tabAgent.appendChild(chatContainer);

  // Chat input area
  const chatInputArea = document.createElement('div');
  chatInputArea.style.cssText = 'display:flex;gap:8px;padding:12px;border-top:1px solid rgba(255,255,255,0.1);';

  const chatInput = document.createElement('input');
  chatInput.id = 'agent-input';
  chatInput.type = 'text';
  chatInput.placeholder = 'Ask me anything about CS2 optimizations...';
  chatInput.style.cssText = 'flex:1;padding:8px 12px;border-radius:6px;background:rgba(0,0,0,0.3);border:1px solid rgba(255,255,255,0.1);color:var(--text);font-size:12px;';

  const btnSend = document.createElement('button');
  btnSend.id = 'agent-send';
  btnSend.className = 'btn-export';
  btnSend.textContent = 'Send';
  btnSend.style.cssText = 'padding:8px 20px;';

  chatInputArea.appendChild(chatInput);
  chatInputArea.appendChild(btnSend);
  tabAgent.appendChild(chatInputArea);

  // Model management section (collapsible)
  const modelSection = document.createElement('details');
  modelSection.style.cssText = 'padding:12px;border-top:1px solid rgba(255,255,255,0.1);';

  const modelSummary = document.createElement('summary');
  modelSummary.style.cssText = 'cursor:pointer;font-size:11px;font-weight:600;opacity:0.7;';
  modelSummary.textContent = '⚙️ Model Management';

  const modelContent = document.createElement('div');
  modelContent.id = 'agent-models';
  modelContent.style.cssText = 'margin-top:12px;display:flex;flex-direction:column;gap:8px;';

  modelSection.appendChild(modelSummary);
  modelSection.appendChild(modelContent);
  tabAgent.appendChild(modelSection);

  // Agent signature
  const agentSig = document.createElement('section');
  agentSig.className = 'cfg-actions';
  agentSig.style.marginTop = 'auto';
  const agentSigText = document.createElement('div');
  agentSigText.className = 'app-signature';
  agentSigText.innerHTML = `<span class="sig-by">by</span> <span class="sig-name">Rqdiniz</span> <span class="sig-tag">[ bu- ]</span>`;
  agentSig.appendChild(agentSigText);
  tabAgent.appendChild(agentSig);
  ...
  container.appendChild(tabAgent);
  ...
  async function initAgent() {
    ...
    try {
      // Initialize AI assistant backend
      await invoke('init_ai_assistant');

      // Load available providers
      const providers = await invoke<any[]>('llm_get_all_providers');
      const sel = document.getElementById('agent-provider') as HTMLSelectElement;
      if (sel && providers) {
        sel.innerHTML = '';
        providers.forEach((p: any) => {
          const opt = document.createElement('option');
          opt.value = p.id;
          opt.textContent = `${p.name} (${p.provider_type}) - ${p.model_name}`;
          opt.disabled = !p.enabled;
          sel.appendChild(opt);
        });

        // Set active provider
        const active = await invoke<any>('llm_get_active_provider');
      ...
      if (statusDot) {
        ...
        statusDot.title = 'Agent ready';
      }

      addAgentMessage('system', 'AI Assistant initialized. How can I help you optimize your CS2 experience?');
    } catch (e) {
      console.error('Failed to initialize agent:', e);
      addAgentMessage('system', `Initialization error: ${e}`);
    }
  }

  function addAgentMessage(role: 'user' | 'assistant' | 'system', content: string) {
    ...
    msgDiv.style.cssText = `
      padding: 12px;
      border-radius: 8px;
      background: ${role === 'user' ? 'rgba(59, 130, 246, 0.15)' : role === 'system' ? 'rgba(100, 116, 139, 0.15)' : 'rgba(16, 185, 129, 0.15)'};
      border: 1px solid ${role === 'user' ? 'rgba(59, 130, 246, 0.3)' : role === 'system' ? 'rgba(100, 116, 139, 0.3)' : 'rgba(16, 185, 129, 0.3)'};
      font-size: 12px;
      line-height: 1.5;
    `;

    const roleSpan = document.createElement('div');
    roleSpan.textContent = role === 'user' ? 'You' : role === 'system' ? 'System' : 'Agent';
    roleSpan.style.cssText = 'font-weight: 600; margin-bottom: 6px; opacity: 0.7; font-size: 10px; text-transform: uppercase;';
  ...
  async function sendAgentMessage() {
    ...
    try {
      // Get active provider
      const provider = await invoke<any>('llm_get_active_provider');
      if (!provider) {
        addAgentMessage('system', 'No LLM provider selected. Please select a provider from the dropdown.');
      ...
      addAgentMessage('system', `Error: ${e}`);
    }
  ...
  function toggleVoice() {
    ...
    const btn = document.getElementById('agent-voice-toggle');
    if (btn) {
      btn.innerHTML = agentVoiceEnabled ? '🎤 Voice: ON' : '🎤 Voice: OFF';
      btn.className = agentVoiceEnabled ? 'btn-export' : 'btn-import';
    }

    if (agentVoiceEnabled) {
      addAgentMessage('system', 'Voice assistant enabled. Press Ctrl+Space to talk.');
      // TODO: Initialize voice handler
    } else {
      addAgentMessage('system', 'Voice assistant disabled.');
    }
  }
  ...
  async function clearAgentChat() {
    try {
      ...
      addAgentMessage('system', 'Chat history cleared.');
    } catch (e) {
  ...
  async function loadModelManagement() {
    const modelsDiv = document.getElementById('agent-models');
    if (!modelsDiv) return;
    ...
    try {
      ...
      providers.forEach((p: any) => {
        ...
        pDiv.style.cssText = 'display:flex;align-items:center;gap:8px;padding:8px;background:rgba(0,0,0,0.2);border-radius:4px;font-size:11px;';

        const icon = document.createElement('span');
        icon.textContent = p.provider_type === 'Local' ? '💻' : '☁️';
        ...
        const statusText = typeof p.status === 'string' ? p.status : 
                          p.status.Downloading !== undefined ? `${Math.round(p.status.Downloading)}%` :
                          p.status.Error ? 'Error' : 'Unknown';
        status.textContent = statusText;
        status.style.cssText = 'font-size:10px;opacity:0.6;';

        const toggleBtn = document.createElement('button');
        toggleBtn.textContent = p.enabled ? 'Disable' : 'Enable';
        toggleBtn.style.cssText = 'padding:4px 8px;border-radius:4px;font-size:10px;cursor:pointer;';
        toggleBtn.onclick = async () => {
          try {
            await invoke('llm_enable_provider', { providerId: p.id, enabled: !p.enabled });
            loadModelManagement();
        ...
        pDiv.appendChild(toggleBtn);
        modelsDiv.appendChild(pDiv);
      });
    } catch (e) {
      modelsDiv.innerHTML = `<span style="color:#ef4444;font-size:11px;">Error loading providers: ${e}</span>`;
    }
  }

  // Agent tab event listeners
  ...
  const providerSelectEl = document.getElementById('agent-provider') as HTMLSelectElement;

  if (agentSendBtn) agentSendBtn.addEventListener('click', sendAgentMessage);
  if (agentInput) {
  ...
  if (clearChatBtn) clearChatBtn.addEventListener('click', clearAgentChat);
  if (providerSelectEl) {
    providerSelectEl.addEventListener('change', async () => {
      try {
        await invoke('llm_set_active_provider', { providerId: providerSelectEl.value });
        toast('Provider switched');
      } catch (e) {
        toast(`Error: ${e}`, true);
      }
    });
  ...
  // Initialize agent when tab is first opened
